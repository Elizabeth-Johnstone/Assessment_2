...
/*
    Third, destructure the price off of the pizza object. Print the value of your new price variable.
*/
...
/*
    Fourth, and last, destructure the category property. Print the value of your category variable. 
*/
...
//////////////////PROBLEM 4////////////////////
/* 
    Let's filter the food objects according to their tags. Write a callback function below that will return only food objects that have a certain tag. You can check for any tag that at least 1 of your food objects has.
*/

const filteredFood = foodArr.filter(element => element.tags.includes('healthy'))
...
const filterByProperty = (property, number, type) => {
    let filteredArray = foodArr.filter(element => {
        if (type === 'above' && element[property] > number) {
            return element
        } else if (type === 'below' && element[property] < number)
            return element
        }
    )
...
/*
    Invoke the `filterByProperty` function passing in a value for each parameter. You'll have to console.log to see the filtered array
*/
...
console.log(filterByProperty('rating', 8.5, 'above'))
